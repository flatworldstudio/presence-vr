...
public class KinectManager : MonoBehaviour
{
    ...
    // FWS. Public bool to determine wether or not to receive the user depth map

    public bool ComputeUserTexture = false;
    ...
    public bool DisplayColorMap = true;
    ...
    private int usersMapSize, usersMapHeight, usersMapWidth;
    ...
    public void WakeUp()
    {
        ...
        if (ComputeUserMap)
        {
            ...
            usersMapHeight = KinectWrapper.GetDepthHeight();
            usersMapWidth = KinectWrapper.GetDepthWidth();
    ...
    void Update()
    {
        if (KinectInitialized)
        {
            ...
            // FWS. Added bool to retrieve depth map.
        ...
        if (KinectInitialized)
        {
            if (ComputeUserMap && (/**(allUsers.Count == 0) ||*/ DisplayUserMap))
            {
                if (usersMapRect.width == 0 || usersMapRect.height == 0)
                {
                    ...
                    Rect cameraRect = CameraDebug.pixelRect;
            ...
            else if (ComputeColorMap && (/**(allUsers.Count == 0) ||*/ DisplayColorMap))
            {
                if (usersClrRect.width == 0 || usersClrTex.height == 0)
                {
                    // get the main camera rectangle
                    Rect cameraRect = Camera.main.pixelRect;
                    //Rect cameraRect = CameraDebug.main.pixelRect;
    ...
    void UpdateUserMap()
    {
        ...
        for (int i = 0; i < usersMapSize; i++)
        {
            ...
            // so kinect depth data is 11 bits. 3 bits user id.
            // 00dd dddd dddd duuu
            ...
            if (nowUserPixel != wasUserPixel)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // Test. create colour based on just depth value. range would appear to be 8192 (13 bits. 16 bit of which right hand 3 bits user to mark user)
                        // this isn raw!!  wrapper appears to give depth in cm...
                        //
                        //						depthMin = Math.Min(userDepth,depthMin);
                        //						depthMax = Math.Max(userDepth,depthMax);

                        //

                        //						Vector3 point;
                        //
                        //						int y = i / usersMapHeight;
                        ...
                        //						int x = i - y * usersMapHeight;
                        //
                        //						if (y == usersMapHeight / 2 && x == usersMapWidth / 2 ) {
                        //
                        //							debugCube.transform.position = depthToWorld(x,y,userDepth);
                        //							//debugCube.transform.position = new Vector3(0,0,userDepth / 1000f);
                        //
                        //							GameObject.Find ("Debug").GetComponent<GUIText> ().text = "depthValue: " + userDepth;
                        //
                        //
                        //
                        //						}

                        float value = userDepth / 4000f;
                        c = new Color(value, value, value, 0.9f);
        ...
        //	Debug.Log ("Depth: " + depthMin + " " + depthMax);
    }
    ...
    {
        return usersMapHeight;
    }

    public int getUserDepthWidth()
    {
        return usersMapWidth;
